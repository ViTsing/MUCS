...
python cherry_selection/data_analysis.py \
    --data_path data/alpaca_data.json \
    --save_path alpaca_data_pre.pt \
    --model_name_or_path /mnt/sdb/yyz/.cache/huggingface/llama-1-7b \
    --max_length 512 \
    --prompt alpaca \
    --mod pre
'''
...
def parse_args():
    ...
    parser.add_argument("--mod", type=str, default='pre', help='pre, cherry')   # 模式：pre表示预处理阶段，只算instruction； cherry表示打分阶段，算回答相关ppl
...
'''pre mod:
{
  'ppl': [ppl_instruction, ppl_output_alone, ppl_output_condition],
  'sent_emb': [embedding_instruction, 0, 0]   
}
'''
...
{
  'ppl': [ppl_instruction, ppl_output_alone, ppl_output_condition],
  'token_loss': [[], loss_list_alone, loss_list_condition]  
}
'''